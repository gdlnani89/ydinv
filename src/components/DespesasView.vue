<script>
import Cabecalho from './Cabecalho.vue';
import BtnAdd from './BtnAdd.vue';
import ModalCadastraDespesa from './ModalCadastraDespesa.vue';
import TotalCard from './TotalCard.vue';
import Periodo from './Periodo.vue';
import { useBalancePrivacy } from '../composables/useBalancePrivacy.js';
import currencyMixin from '../mixins/currencyMixin.js';

export default{
  mixins: [currencyMixin],
  setup() {
    const { isBalanceHidden, toggleBalancePrivacy, formatBalanceWithPrivacy } = useBalancePrivacy();
    return {
      isBalanceHidden,
      toggleBalancePrivacy,
      formatBalanceWithPrivacy
    };
  },
  props: ['contas', 'despesas'],
  data(){
    return{
      modalDespesa: false,
      dia: new Date().getDate(),
      contasExpandidas: [],
      contaEmEdicao: null,
      periodoSelecionado: '',
    }
  },
  components:{
    Cabecalho,
    BtnAdd,
    ModalCadastraDespesa,
    TotalCard,
    Periodo,
  },
  methods: {
    abrirModalDespesa(){
      this.modalDespesa = true;
    },
    fecharModalDespesa(){
      this.modalDespesa = false;
    },
    atualizarPeriodo(novoPeriodo) {
      this.periodoSelecionado = novoPeriodo;
    },

    formatarData(data) {
      // Formata a data de forma segura (evita problemas de fuso horário)
      const [ano, mes, dia] = data.split('-');
      return `${dia}/${mes}/${ano}`;
    },

    
    ativarModoEdicao(contaId) {
      if (this.contaEmEdicao === contaId) {
        this.contaEmEdicao = null;
      } else {
        this.contaEmEdicao = contaId;
      }
    },
    
    expandirConta(contaId) {
      if (this.contasExpandidas.includes(contaId)) {
        this.contasExpandidas = this.contasExpandidas.filter(id => id !== contaId);
      } else {
        this.contasExpandidas.push(contaId);
      }
    },
    
    editarDespesa(despesa) {
      this.$emit('editar-despesa', despesa);
    },
    
    excluirDespesa(despesa) {
      if (confirm(`Tem certeza que deseja excluir a despesa "${despesa.descricao}"?`)) {
        this.$emit('excluir-despesa', despesa.id);
      }
    },
    formatarPeriodo(periodo) {
      if (!periodo) return '';
      const [ano, mes] = periodo.split('-');
      const meses = [
        '', 'Janeiro', 'Fevereiro', 'Março', 'Abril', 'Maio', 'Junho',
        'Julho', 'Agosto', 'Setembro', 'Outubro', 'Novembro', 'Dezembro'
      ];
      return `${meses[parseInt(mes, 10)]}/${ano}`;
    },
  },
  mounted() {
    // Não precisa carregar do localStorage aqui, pois as despesas vêm como prop
  },
  computed:{
    // Retorna todas as despesas
    despesasFiltradas() {
      if (!this.periodoSelecionado) return this.despesas;
      const [anoSelecionado, mesSelecionado] = this.periodoSelecionado.split('-');
      return this.despesas.filter(despesa => {
        const [anoDespesa, mesDespesa] = despesa.data.split('-');
        return anoDespesa === anoSelecionado && mesDespesa === mesSelecionado;
      });
    },
    
    despesasPorConta() {
      const despesasAgrupadas = {};
      
      this.contas.forEach(conta => {
        despesasAgrupadas[conta.id] = this.despesasFiltradas.filter(despesa => despesa.contaId === conta.id);
      });
      
      return despesasAgrupadas;
    },
    
    saldosPorConta() {
      const saldos = {};
      
      this.contas.forEach(conta => {
        const despesasDaConta = this.despesasFiltradas.filter(despesa => despesa.contaId === conta.id);
        const saldo = despesasDaConta.reduce((total, despesa) => {
          return total + this.safeParseFloat(despesa.valor);
        }, 0);
        saldos[conta.id] = saldo;
      });
      
      return saldos;
    },
    
    totalDespesas() {
      return this.despesasFiltradas.reduce((total, despesa) => {
        return total + this.safeParseFloat(despesa.valor);
      }, 0);
    },
  }
}

</script>

<template>
  <section class="content-section" id="despesas">
    <!-- Área fixa do topo -->
    <div class="pb-2 bx-shadow sticky-top bg1">
      <div class="d-flex justify-content-between align-items-center py-2 px-3 bx-shadow mb-3">
        <h4>Despesas</h4>
        <div class="d-flex align-items-center gap-2">
          <button 
            @click="toggleBalancePrivacy"
            class="btn btn-sm btn-outline-secondary"
            :title="isBalanceHidden ? 'Mostrar saldos' : 'Ocultar saldos'"
          >
            <i :class="['bi', isBalanceHidden ? 'bi-eye-fill' : 'bi-eye-slash-fill']"></i>
          </button>
        </div>
      </div>
      <Periodo v-model="periodoSelecionado" @periodo-change="atualizarPeriodo" />
      <TotalCard
        class="m-3" 
        titulo="Total de Despesas"
        :valor="totalDespesas"
        :contador="despesasFiltradas.length"
        itemLabel="despesa"
        tipo="despesas"
      />
      

    </div>
    
    <!-- Área rolável das contas -->
    <div class="scrollable-content">
      <!-- Lista de contas -->
      <div class="contas-grid" v-if="contas.length > 0">
      <div 
        v-for="conta in contas" 
        :key="conta.id" 
        class="conta-card bg1"
        :style="{ '--conta-cor': conta.cor }"
      >
        <div class="conta-header">
          <div>
            <h5 class="conta-nome">{{ conta.nome }}</h5>
            <span class="conta-saldo">{{ formatBalanceWithPrivacy(saldosPorConta[conta.id]) }}</span>
          </div>

          <div class="conta-acoes">
            <!-- Botão para expandir/recolher a conta -->
            <button 
              class="btn btn-sm btn-outline-secondary"
              @click="expandirConta(conta.id)"
              :title="contasExpandidas.includes(conta.id) ? 'Recolher' : 'Expandir'"
            >
              <i :class="['bi', contasExpandidas.includes(conta.id) ? 'bi-chevron-up' : 'bi-chevron-down']"></i>
            </button>
            <!-- Botão para ativar modo de edição dos lançamentos (só aparece quando expandido) -->
            <button 
              v-if="contasExpandidas.includes(conta.id)"
              class="btn btn-sm"
              :class="contaEmEdicao === conta.id ? 'btn-warning' : 'btn-outline-warning'"
              @click="ativarModoEdicao(conta.id)"
              :title="contaEmEdicao === conta.id ? 'Sair do modo edição' : 'Editar lançamentos'"
            >
              <i class="bi bi-pencil-square"></i>
            </button>
          </div>
        </div>
        
        <!-- Lista de despesas da conta (mostra apenas se expandida) -->
        <div v-if="contasExpandidas.includes(conta.id)">
          <div v-if="despesasPorConta[conta.id] && despesasPorConta[conta.id].length > 0" class="despesas-lista">
            <h6 class="despesas-titulo">Despesas:</h6>
            <div v-for="despesa in despesasPorConta[conta.id]" :key="despesa.id" class="despesa-item">
              <div class="despesa-info">
                <span class="despesa-descricao">{{ despesa.descricao }}</span>
                <span class="despesa-data">{{ formatarData(despesa.data) }}</span>
              </div>
              <div class="despesa-acoes">
                <span class="despesa-valor">{{ formatCurrency(despesa.valor) }}</span>
                <!-- Controles de edição (aparecem apenas no modo de edição) -->
                <div v-if="contaEmEdicao === conta.id" class="despesa-controles">
                  <button 
                    class="btn btn-sm btn-outline-primary"
                    @click="editarDespesa(despesa)"
                    title="Editar despesa"
                  >
                    <i class="bi bi-pencil"></i>
                  </button>
                  <button 
                    class="btn btn-sm btn-outline-danger"
                    @click="excluirDespesa(despesa)"
                    title="Excluir despesa"
                  >
                    <i class="bi bi-trash"></i>
                  </button>
                </div>
              </div>
            </div>
          </div>
          
          <div v-else class="sem-despesas">
            <p class="text-muted small">Nenhuma despesa cadastrada</p>
          </div>
        </div>
        
        <!-- Indicador de despesas quando não expandida -->
        <div v-else-if="despesasPorConta[conta.id] && despesasPorConta[conta.id].length > 0" class="despesas-indicator">
          <small class="text-muted">
            <i class="bi bi-list-ul me-1"></i>
            {{ despesasPorConta[conta.id].length }} despesa{{ despesasPorConta[conta.id].length !== 1 ? 's' : '' }}
          </small>
        </div>
        
      </div>
    </div>

    <!-- Mensagem quando não há contas -->
    <div v-else class="text-center py-5">
      <i class="bi bi-bank fs-1 text-muted mb-3"></i>
      <h5 class="text-muted">Nenhuma conta cadastrada</h5>
      <p class="text-muted">Clique em "Adicionar Conta" para começar</p>
    </div>

    <!-- Mensagem quando não há despesas no período -->
    <div v-if="contas.length > 0 && despesasFiltradas.length === 0 && periodoSelecionado" class="text-center py-5">
      <i class="bi bi-calendar-x fs-1 text-muted mb-3"></i>
      <h5 class="text-muted">Nenhuma despesa em {{ formatarPeriodo(periodoSelecionado) }}</h5>
      <p class="text-muted">Adicione uma despesa para este período ou altere o período selecionado</p>
    </div>

      <BtnAdd tipo="despesa" @abrir-modal-despesa="abrirModalDespesa"/>

      <ModalCadastraDespesa 
        v-show="modalDespesa"
        :contas="contas"
        :periodo="periodoSelecionado"
        @fechar-modal-despesa="fecharModalDespesa"
        @adicionar-despesa="$emit('adicionar-despesa', $event)"
      />
    </div>
  </section>
</template>

<style scoped>

.header-section {
  flex-shrink: 0;
  padding: 0.75rem;
  background: white;
  border-bottom: 1px solid #e9ecef;
}

.scrollable-content {
  flex: 1;
  overflow-y: auto;
  padding: 1rem;
}

.contas-grid {
  display: grid;
  grid-template-columns: repeat(auto-fill, minmax(280px, 1fr));
  gap: 1rem;
  margin: 0;
}

.conta-card {
  background: white;
  border-radius: 12px;
  padding: 1.5rem;
  box-shadow: 0 2px 15px rgba(0, 0, 0, 0.08);
  border-left: 4px solid var(--conta-cor);
  transition: transform 0.2s ease, box-shadow 0.2s ease;
  position: relative;
}

.conta-card:hover {
  transform: translateY(-2px);
  box-shadow: 0 4px 20px rgba(0, 0, 0, 0.12);
}

.conta-header {
  margin-bottom: 1rem;
  display: flex;
  justify-content: space-between;
  position: relative;
}

.conta-nome {
  font-size: 1.1rem;
  font-weight: 600;
  margin: 0;
  color: #333;
}

.conta-saldo {
  font-size: 1.5rem;
  font-weight: 700;
  color: var(--conta-cor);
  display: block;
  margin-top: 0.5rem;
}

.conta-acoes {
  display: flex;
  justify-content: flex-end;
  gap: 0.5rem;
}

.btn-sm {
  padding: 0.25rem 0.5rem;
  font-size: 0.875rem;
}

.despesas-indicator {
  margin-top: 0.5rem;
  padding: 0.5rem 0;
  border-top: 1px solid #eee;
}

.despesas-lista {
  margin-top: 1rem;
  padding-top: 1rem;
  border-top: 1px solid #eee;
}

.despesas-titulo {
  font-size: 0.9rem;
  font-weight: 600;
  color: #555;
  margin-bottom: 0.5rem;
}

.despesa-item {
  display: flex;
  justify-content: space-between;
  align-items: center;
  padding: 0.5rem 0;
  border-bottom: 1px solid #f5f5f5;
}

.despesa-item:last-child {
  border-bottom: none;
}

.despesa-info {
  flex-grow: 1;
  margin-right: 1rem;
}

.despesa-acoes {
  display: flex;
  align-items: center;
  gap: 0.5rem;
}

.despesa-controles {
  display: flex;
  gap: 0.25rem;
}

.despesa-descricao {
  font-size: 0.9rem;
  color: #333;
  font-weight: 500;
}

.despesa-data {
  font-size: 0.75rem;
  color: #888;
  margin-left: 0.5rem;
}

.despesa-valor {
  font-size: 0.9rem;
  font-weight: 600;
  color: var(--conta-cor);
}

.sem-despesas {
  text-align: center;
  padding: 1rem;
  color: #888;
}


</style>